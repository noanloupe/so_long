...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/15 11:56:26 by noloupe           #+#    #+#             */
/*   Updated: 2022/12/15 12:06:00 by noloupe          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd > OPEN_MAX || fd < 0 || BUFFER_SIZE < 1)
		return (NULL);
	line = save_dup(save[fd], &size, &check);
	if (!line)
		return (NULL);
	copy(save[fd], &save[fd][size], BUFFER_SIZE + 1);
	line = fct_get_line(line, save[fd], &check, fd);
	if (!line || *line == '\0')
	{
		free(line);
		return (NULL);
